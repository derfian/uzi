...
;  tell %{tank} &+CAGRO&+ccop &+W[&+ROFF&+W] &+CRE&+ccop &+W[&+GON&+W]%;\
  /else  \
	  tell %{tank} &+cAutomatically copping &+Ragro &+crooms.%;\
;    tell %{tank} &+CAGRO&+ccop &+W[&+GON&+W] &+CRE&+ccop &+W[&+ROFF&+W]%;\
  /endif%;/endif
  
/def acop = \
     /if (coptype=4) /echo -aBCgreen 1. Someone else will do the copping.%;/set autocop=0%;/set acop=0%;/set coptype=1%;\
     /elseif (coptype=1) /echo -aBCcyan 2. Automatically copping + keeping cops up.%;/set autocop=1%;/set acop=1%;/set coptype=2%;\
     /elseif (coptype=2) /echo -aBCcyan 3. Keeping cops if they bail.%;/set acop=1%;/set autocop=0%;/set coptype=3%;\
     /else /echo -aBCcyan 4. Automatically copping agro rooms.%;/set acop=0%;/set autocop=1%;/set coptype=4%;/endif
