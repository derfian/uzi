...
; from here
;  /else \
;    /ecko Not assisting. Sent assist already%;\
  /endif%;\
  /set groupass=0
...
;          /set aggmob=$[{aggmob}-1]%;\
;          /if ((aggmob!=0)&(assist=1)) \
;            /ecko Still %{htxt2}%{aggmob} aggs%{ntxt}! Assisting %{htxt2}group!%;\
;            /doassist %{tank}%;\
;          /endif%;\
;        /else \
...
/def -F -p1 -mregexp -t'([A-Za-z]+) (massacres|obliterates|annihilates|vaporizes|pulverizes|atomizes|ultraslays|\*\*\*ULTRASLAYS\*\*\*) ([A-Za-z]+)' autoassist1 = \
        /assist %{P3} %{P1}
